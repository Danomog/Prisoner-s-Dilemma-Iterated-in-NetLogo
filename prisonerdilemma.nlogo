...
turtles-own [
  ...
  cooperare?
  avversario
  avversario-ha-cooperato?
]

to setup
  clear-all
  set-default-shape plas "question"
  set-default-shape plbs "question"
  create-plas 1 [
    set heading 270
    fd max-pxcor / 2
    set color 95
  ...
  ask turtles [
    set size 10
    set avversario one-of other turtles

    set cooperare? true
    set avversario-ha-cooperato? true
    set punti 0
  ]
  reset-ticks
  ask turtles [ set label punteggio-medio ]
end

to-report punteggio-medio
  report precision (punti / (ticks + 1)) 5  ;ticks + 1 perchè se no da errore, dice che divido per zero (?)
end
...
to turno
  ask plas [
    ...
    run strategia-giocatore-a
  ]
  ask plbs [
    let shb strategia-giocatore-b
    (ifelse
      shb = "Randomizzato"[
      set shape "mouse side"
      ]
    ...
    run strategia-giocatore-b
  ]
  ask turtles [ calcola-punti ]
end

to calcola-punti
  set avversario-ha-cooperato? [ cooperare? ] of avversario
  ifelse avversario-ha-cooperato?
    [ ifelse cooperare?
      [ set punti punti + 3 ]
      [ set punti punti + 5 ]
    ]
...
to randomizzato
  set cooperare? one-of [ true false ]
end
...
to permaloso
  ifelse not avversario-ha-cooperato? or not cooperare?
    [ set cooperare? false ]
    [ set cooperare? true ]
end
...
152
55
Strategia-Giocatore-A
Strategia-Giocatore-A
"Randomizzato" "Cooperante" "Traditore" "Permaloso" "Specchio"
0
...
"" ""
PENS
"Giocatore A" 1.0 0 -13791810 true "" "plot [ punteggio-medio ] of one-of plas"
"Giocatore B" 1.0 0 -2064490 true "" "plot [ punteggio-medio ] of one-of plbs"
...
"" ""
PENS
"Giocatore A" 1.0 0 -13791810 true "" "plot [ punti ] of one-of plas"
"Giocatore B" 1.0 0 -2064490 true "" "plot [ punti ] of one-of plbs"
...
0
Circle -7500403 true true 0 0 300
Polygon -16777216 true false 75 45 210 45 225 75 225 135 195 165 165 195 165 225 120 225 120 180 195 120 195 90 180 75 75 75 75 45 75 45 105 45
Polygon -16777216 true false 120 255 120 285 165 285 165 255 120 255
Polygon -16777216 true false 75 45 105 30 195 30 210 45 75 45
Polygon -7500403 true true 105 75 105 60 120 45 180 45 195 75 195 90 105 90 105 75
